...
import Utils.classification_data_utils as data_utils
from Utils.classification_data_utils import ABSATokenizer, TargetPooledClassification
...
def train(args):
    ...
    tokenizer = ABSATokenizer.from_pretrained(args.pretrained_model_dir)
    ...
    all_target_ids = torch.tensor([f.target_indices for f in train_features], dtype=torch.long)

    train_data = TensorDataset(all_input_ids, all_segment_ids, all_input_mask, all_label_ids, all_target_ids)
    ...
    #>>>>> validation
    if args.do_valid:
        valid_examples = processor.get_conll_examples(args.dev_data, "dev")
        ...
        valid_all_target_ids = torch.tensor([f.target_indices for f in valid_features], dtype=torch.long)
        valid_data = TensorDataset(valid_all_input_ids, valid_all_segment_ids, valid_all_input_mask, valid_all_label_ids, valid_all_target_ids)
    ...
    model = TargetPooledClassification(args.pretrained_model_dir,
                                       hidden_dropout_prob=0.3,
                                       num_labels=len(label_list),
                                       pool_target=args.pooling)
    model.cuda()
    # Prepare optimizer
    ...
    for _ in range(args.num_train_epochs):
        ...
        if args.do_valid:
            ...
            with torch.no_grad():
                ...
                preds = []
                golds = []
                ids = [str(e.guid) for e in valid_examples]
                ...
                valid_acc = accuracy_score(golds, preds)
                valid_f1 = f1_score(golds, preds, average="macro")

                logger.info("validation loss: %f", valid_loss)
                logger.info("validation acc: %f", valid_acc)
                logger.info("validation macro F1: %f", valid_f1)
            ...
            model.train()
    if args.do_valid:
        with open(os.path.join(args.output_dir, "dev_preds.json"), "w") as fw:
...
def test(args):  # Load a trained model that you have fine-tuned (we assume evaluate on cpu)
    ...
    eval_examples = processor.get_conll_examples(args.test_data, "test")
    ...
    with open(output_eval_json, "w") as fw:
        json.dump({"logits": full_logits, "predictions": predictions,
                   "golds": golds, "acc": test_acc, "f1": test_f1,
                   "ids": ids, "targets": targets, "texts": texts}, fw)
...
if __name__=="__main__":
    ...
    parser.add_argument("--pooling",
                        default='cls',
                        type=str,
                        help="cls, first, or pooled (default is CLS)")

    parser.add_argument("--pretrained_model_dir",
                        default="finetuned_models/all",
                        type=str,
                        required=True,
                        help="The location of the domain-tuned bert model")

    parser.add_argument("--train_data",
                        default=None,
                        type=str,
                        required=True,
                        help="The train input conll.")

    parser.add_argument("--dev_data",
                        default=None,
                        type=str,
                        required=True,
                        help="The dev input conll.")

    parser.add_argument("--test_data",
                        default=None,
                        type=str,
                        required=True,
                        help="The test input conll.")
