#!/bin/bash
#
#SBATCH --job-name=target-extraction --account=nn9447k
#SBATCH --output=target-extraction-%j.out
#
...
OUTPUT="extraction_results/$ann_type"/$dataset
...
for run in 1 2 3 4 5; do
    seed=${SEEDS[$run]}
    python target_extraction.py --annotation_type="$ann_type" --train_data="$TRAIN_FILE" --dev_data="$DEV_FILE" --test_data="$TEST_FILE" --bert_model=bert-base-cased --output_dir="$OUTPUT"/"$run" --trained_model_dir=finetuned_models/"$domain_tuned" --do_train --do_eval --do_test --save_all_epochs --num_train_epochs 50 --seed "$seed" --add_tags
done;
